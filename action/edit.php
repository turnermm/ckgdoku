...
class action_plugin_ckgdoku_edit extends DokuWiki_Action_Plugin {
    ...
    * @author Myron Turner <turnermm03@shaw.ca>     
     ...
     * @return void
     */
    function ckgdoku_edit_meta(Doku_Event $event)
    {
        global $ACT;
        ...
        $event->data['script'][] = 
            array( 
                ...
                'charset'=>'utf-8', 
                '_data'=>'',
                 'src'=>DOKU_BASE.'lib/plugins/ckgdoku/' .$this->fck_location. '/ckeditor.js'
            );
     ...
     * @return void
     */
    function ckgdoku_edit(Doku_Event $event)
    {
    ...
    */
    ...
    if($this->getConf('smiley_hack')) {
        ...
        $text=preg_replace("#(?<=http://)(.*?)(?=lib/plugins/ckgdoku/ckeditor/plugins/smiley/images)#s", $new_addr,$text);        
     }
         ...
         */
          ...
          $text = preg_replace_callback(
             '/~~START_HTML_BLOCK~~.*?CLOSE_HTML_BLOCK/ms',
                 create_function(
                '$matches',
                '$matches[0] = str_replace("_ckgdoku_NPBBR_","",$matches[0]);
                 return $matches[0];'
        ),$text);    
...
$ckeditor_replace =<<<CKEDITOR_REPLACE
		   ...
		   ckgdokuCKInstance = CKEDITOR.replace('wiki__text',
		       { 
                  toolbar: '$toolbar' ,    
                  height: $height,
           ...
           FCKeditor_OnComplete(ckgdokuCKInstance);
...
?>
   ...
   <form id="dw__editform" method="post" action="<?php echo script()?>"  accept-charset="<?php echo $lang['encoding']?>">
    <div class="no">
      ...
      <input type="hidden" id="ckgdoku_mode_type"  name="mode" value="" />
...
$is_ckgdokuChrome = false;
 if(stripos($_SERVER['HTTP_USER_AGENT'],'Chrome') !== false) {
      ...
      if((int)$cmatch[1] <26)  $is_ckgdokuChrome =true;    
} 
    ...
    <div id="wiki__editbar">
      ...
      <?php if($wr){?>
         <div class="editButtons">
            <input type="checkbox" name="ckgdoku" value="ckgdoku" checked="checked" style="display: none"/>
...
<?php                  
if($is_ckgdokuChrome) echo $chrome_dwedit_link;
?>
 <br />
   ...
   <div id = "backup_msg" class="backup_msg" style=" display:none;">
     ...
     <label class="nowrap" for="complex_tables" id="complex_tables_label">     
      ...
      <input style="display:none;" class="button" id="edbtn__save" type="submit" name="do[save]" 
            ...
            <!-- Not used by ckgdoku but required to prevent null error when DW adds events -->
            <input type="button" id='edbtn__preview' style="display: none"/>


 <div id='saved_wiki_html' style = 'display:none;' ></div>
 <div id='ckgdoku_draft_html' style = 'display:none;' >
...
?>  
   ...
   var ckgdoku_draft_btn = "<?php echo $this->getLang('btn_exit_draft') ?>";
   var ckgdoku_draft_btn_title = "<?php echo $this->getLang('title_exit_draft')?>";
   function ckgdoku_get_draft() {
      var dom = GetE('ckgdoku_draft_html');
      ...
      var btn = GetE('ckgdoku_draft_btn');
      var tmp = btn.value;  
      btn.value = ckgdoku_draft_btn;
      ckgdoku_draft_btn = tmp;
      tmp = ckgdoku_draft_btn_title;
      btn.title = ckgdoku_draft_btn_title;
      ckgdoku_draft_btn_title = tmp;
   }
...
var ckgdoku_xcl_fonts =parseInt  ("<?php echo $this->getConf('font_options') ;?>");
var ckgdoku_xcl_colors =parseInt("<?php echo $this->getConf('color_options') ;?>");
var ckgdoku_xcl_styles = (ckgdoku_xcl_fonts + ckgdoku_xcl_colors ==2) ? true : false;
var HTMLParser_DEBUG = "";
var ckgdoku_hasCaptcha = "<?php echo $this->captcha?1:0?>";
...
<?php  
  ...
  if($this->test) {
     ...
     $parse_url = DOKU_URL . 'lib/plugins/ckgdoku/scripts/parse_wiki.js.unc?n=' . $nval;
  }
  else $parse_url = DOKU_URL . 'lib/plugins/ckgdoku/scripts/parse_wiki-cmpr.js';
  
  echo "var parse_url = '$parse_url';";
//  $safe_url = DOKU_URL . 'lib/plugins/ckgdoku/scripts/safeFN_cmpr.js';       
?>
...
if(window.DWikifnEncode && window.DWikifnEncode == 'safe') {
   LoadScript(DOKU_BASE + 'lib/plugins/ckgdoku/scripts/safeFN_cmpr.js' );
}
...
$text = preg_replace_callback(
    ...
    $text);    
        ...
        // prevents utf8 conversions of quotation marks
         $text = str_replace('"',"_ckgdoku_QUOT_",$text);        

         $text = preg_replace_callback('/(<code.*?>)([^<]+)(<\/code>)/ms',
             create_function(
               '$matches',              
               '$quot =  str_replace("_ckgdoku_QUOT_",\'"\',$matches[2]); 
                $quot = str_replace("\\\\ ","_ckgdoku_NL",$quot); 
                $quot .= "_ckgdoku_NL";                
         ...
         $text = preg_replace_callback('/(<file.*?>)([^<]+)(<\/file>)/ms',
          ...
          $text = preg_replace_callback('/(<code>|<file>)([^<]+)(<\/code>|<\/file>)/ms',
             create_function(
               '$matches',             
               '$matches[2] = str_replace("&lt;font","ckgdokuFONTOpen",$matches[2]);
               $matches[2] = str_replace("font&gt;","ckgdokuFONTClose",$matches[2]);
                return $matches[1] .$matches[2] . $matches[3]; '
          ), $text); 
        ...
        */
        ...
        $xhtml = preg_replace('/<p>\s+_ckgdoku_NPBBR_\s+<\/p>/ms',"\n",$xhtml);
        $xhtml = str_replace('_ckgdoku_NPBBR_', "<span class='np_break'>&nbsp;</span>", $xhtml);
  ...
  function write_debug($what) {
     return;
     $handle = fopen("ckgdoku_php.txt", "a");
